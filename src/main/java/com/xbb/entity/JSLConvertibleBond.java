...
public class JSLConvertibleBond implements AbstractTableEntity {
    ...
    @TableProperty(name = BondConstant.DB_LOW, index = 4)
    private String dblow; // 双低值

    @TableProperty(name = BondConstant.BOND_INCREASE_RATE, index = 5)
    ...
    @TableProperty(name = BondConstant.YTM_RT, index = 7)
    private String ytmRt; // 到期收益率

    @TableProperty(name = BondConstant.CURR_ISS_AMT, index = 8)
    private String currIssAmt; // 剩余规模

    @TableProperty(name = "剩余年限", index = 9)
    private String yearLeft; // 剩余年限

    @TableProperty(name = "换手率", index = 10)
    private String turnoverRt; // 换手率

    @TableProperty(name = "评级", index = 11)
    private String ratingCd; // 评级

    @TableProperty(name = BondConstant.STOCK_PRICE, index = 12)
    private String sprice; // 股票价格

    @TableProperty(name = BondConstant.STOCK_PB, index = 13)
    private String pb; // 正股pb

    @TableProperty(name = BondConstant.CONVERT_PRICE, index = 14)
    private String convertPrice; // 转股价

    @TableProperty(name = BondConstant.CONVERT_VALUE, index = 15)
    private String convertValue; // 转股价值

    @TableProperty(name = BondConstant.STOCK_NAME, index = 16)
