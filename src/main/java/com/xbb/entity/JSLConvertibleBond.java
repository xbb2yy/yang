...
public class JSLConvertibleBond implements AbstractTableEntity {
    ...
    @TableProperty(name = BondConstant.STOCK_NAME, index = 5)
    ...
    @TableProperty(name = BondConstant.STOCK_PB, index = 8)
    private String pb; // 正股pb

    @TableProperty(name = BondConstant.CONVERT_PRICE, index = 9)
    private String convertPrice; // 转股价

    @TableProperty(name = BondConstant.CONVERT_VALUE, index = 10)
    private String convertValue; // 转股价值

    @TableProperty(name = BondConstant.PREMIUM_RT, index = 11)
    private String premiumRt; // 溢价率

    @TableProperty(name = BondConstant.DB_LOW, index = 12)
    private String dblow; // 双低值

    @TableProperty(name = "评级", index = 13)
    private String ratingCd; // 评级

    @TableProperty(name = "剩余年限", index = 14)
    private String yearLeft; // 剩余年限

    @TableProperty(name = "换手率", index = 15)
    ...
    @TableProperty(name = BondConstant.LAST_TIME, index = 18)
